(function () {
    ...
    class CContentScript {
        constructor(port) {
            this.m_bValid = true;
            this.m_fnOnWindowMessage = (ev) => {
                if (!this.m_bValid)
                    return;
                if (IsPageMessage(ev.data)
                    && EPageMessage.IMPORTDONE === ev.data.m_emsg) {
                    this.m_port.postMessage(ev.data);
                }
            };
            ...
            style.textContent =
                ".A" + g_strCssClass + " {" +
                    ...
                    "}" +
                    ".A" + g_strCssClass + ":hover {" +
                    "background-color: rgba(0, 114, 0, 0.3);" +
                    "background-position: right top;" +
                    "background-repeat: no-repeat;" +
                    "background-size: 32px 32px;" +
                    "background-image: url(\"" + chrome.runtime.getURL("checkmark.blob.png") + "\");" +
                    "}" +
                    ".A" + g_strCssClass + ":after {" +
                    ...
                    "cursor: pointer;" +
                    "}";
            style.id = "style" + g_strCssClass;
            (document.head || document.documentElement).appendChild(style);
            {
                const script = document.createElement("script");
                script.id = g_strCssClass;
                script.src = chrome.runtime.getURL("inject.js");
                (document.head || document.documentElement).appendChild(script);
            }
            this.m_port.onDisconnect.addListener(this.m_fnOnDisconnect);
            window.addEventListener("message", this.m_fnOnWindowMessage);
        }
        destructor() {
            _ASSERT(this.m_bValid);
            window.removeEventListener("message", this.m_fnOnWindowMessage);
            this.m_port.onDisconnect.removeListener(this.m_fnOnDisconnect);
            const style = document.getElementById("style" + g_strCssClass);
            if (style)
                style.remove();
            this.m_bValid = false;
            window.postMessage(new SPageMessage(EPageMessage.DTOR), "*");
        }
    }
    ;
    function OnConnect(port) {
        chrome.runtime.onConnect.removeListener(OnConnect);
        new CContentScript(port);
    }
